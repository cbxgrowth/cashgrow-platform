...
serve(async (req) => {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [
        {
          price_data: {
            ...
            product_data: { 
              ...
              description: planData.description || `Assinatura do plano ${planData.name}`
            },
      ...
      mode: "subscription",
      success_url: `${req.headers.get("origin")}/client/plans?success=true`,
      cancel_url: `${req.headers.get("origin")}/client/plans?canceled=true`,
      metadata: {
        user_id: user.id,
